...
pub use rrobin::RrobinPolicy;
pub use min_latencia::MintimePolicy;
pub use requisitos::Requisitos;
pub use detour::DetourPolicy;
pub use sed::SedPolicy;
...
type TritonRequest = Request<TritonContext>;
type TritonEndpoint = Endpoint<TritonContext>;
type TritonEndpoints = Endpoints<TritonContext>;

impl RequestContext for TritonContext {
...
pub fn read_models(path: &str) -> Result<Vec<Model>> {
    ...
    if hw_info.gpus.first().is_some_and(|gpu| gpu.name == "Orin" && gpu.core_count == 1024) {

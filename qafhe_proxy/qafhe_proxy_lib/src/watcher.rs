...
use crate::{ProxyMessage, ENDPS_ANNOT};
...
impl AnnotationsWatcher {
    ...
    pub fn new(
        ...
        pod_name: &str,
        namespace: &str,
        sender: mpsc::Sender<ProxyMessage<'static>>
    ) -> Self 
    {
    ...
    fn run(&mut self, sender: mpsc::Sender<ProxyMessage<'static>>) {
                ...
                .try_for_each(|pod| async move {
                    ...
                    if let Some(annots) = pod.metadata.annotations {     
                        ...
                        if let Some(endpoints) = annots.get(ENDPS_ANNOT) {
                            ...
                            if previous_annots.get(ENDPS_ANNOT).is_none_or(|prev_endps| *prev_endps != *endpoints) {
                                
                                match serde_json::from_str(endpoints) {
                                    Ok(new_endpoints) => {
                                        sender.send(ProxyMessage::EndpointsChanged(new_endpoints)).await.unwrap();

...
use kube::Client;
use metrics::Metric;
use policy::{ProxyPolicy, RequestContext};
...
type MsgSender<'a> = mpsc::Sender<ProxyMessage<'a>>;
#[derive(Debug)]
enum ProxyMessage<'a> {
...
where P: ProxyPolicy<R> + 'static,
      R: RequestContext + 'static
{
    let hw_info = get_hardware_info(); 
    log::info!("Detected hardware: {:?}", hw_info);

    let (sender, mut receiver) = tokio::sync::mpsc::channel::<ProxyMessage>(32);
    ...
    loop {
        ...
        match message {
            ProxyMessage::EndpointsChanged(endpoints) => {
                proxy_server.update_endpoints(endpoints);
            },
            ProxyMessage::AnnotationUpdate(annots) => {
                watcher.add_annot(annots).await;
            }
            ProxyMessage::NeighborAnnotRequest { neighbor_name, annot_name, respond_to } => {

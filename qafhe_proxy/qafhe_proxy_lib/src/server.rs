...
use std::{
    collections::BTreeMap, env, net::SocketAddr, str::FromStr, sync::{atomic::{AtomicU32, Ordering}, Arc}, time::{Duration, Instant}
};
use tokio::{
...
use crate::{
    policy::{ProxyPolicy, Receiver, Request, RequestContext, Sender},
    ProxyMessage, MsgSender, HW_ANNOT, METRICS_ANNOT
};
      ...
      R: RequestContext + 'static
{
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (uuid, ep) in endpoints.iter() {
                    ...
                    if ep.metrics_queried_at.is_none_or(|t| t.elapsed() >= QUERY_MAX_ELLAPSED) {
        ...
        -> Result<()>
    {
        ...
        let target_uuid = self.policy.choose_target(&request, &read_handle).await;
        let target = read_handle.get(&target_uuid).context("Endpoint dropped")?;
        
        let target_addr = SocketAddr::from_str(&target.ip).expect("Invalid IP address for node.");
        target.pending_request.fetch_add(1, Ordering::SeqCst); // Decrease pending requests.
        ...
        let result = if target_uuid != self.self_uuid {
            ...
            let res = timeout(self.request_timeout, proxy(&mut client_conn, target_addr, &request))
                .await
                .ok();
            
            let read_handle = self.endpoints.read().await;
            let target = read_handle.get(&target_uuid)
                .context("Endpoint dropped")?;
            target.pending_request.fetch_sub(1, Ordering::SeqCst); // Decrease pending requests.
            res
        }
        else {
        ...
        // On timeout or error, store the instant of the error.
        let now = Instant::now();
    ...
    fn query_annot(self: &Arc<Self>, pod_uuid: Uuid, annot_name: &str, pod_name: &Arc<str>) {
        ...
        tokio::spawn(async move {
            ...
            let message = ProxyMessage::NeighborAnnotRequest { 

...
use anyhow::Result;
use crate::{ProxyMessage, METRICS_ANNOT};
...
impl PrometheusClient {
    ...
    pub fn new(server: &str, target_metrics: Vec<Metric>, sender: mpsc::Sender<ProxyMessage<'static>>) -> Result<Self> {
    ...
    fn run(&mut self, server: &str, target_metrics: Vec<Metric>, sender: mpsc::Sender<ProxyMessage<'static>>) -> Result<()> {
            ...
            loop {
                ...
                match Self::query_metrics(client, &target_metrics).await {
                    Ok(metrics) => {
                        ...
                        sender.send(ProxyMessage::AnnotationUpdate(vec![(METRICS_ANNOT, json)]))
                            .await
